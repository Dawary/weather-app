...
async function getCurrentWeather(city) {
  ...
  try {
    ...
    visibility.innerHTML = `<h4>${(data.visibility / 1000).toFixed(1)}<sub>km</sub></h4>`;
    feelsLike.innerHTML = Math.floor(data.main.feels_like);
    minmax.innerHTML = `<h4>Min:${Math.floor(data.main.temp_min)}</span>&deg;<sup>c</sup> / Max: ${Math.floor(data.main.temp_max)}</span>&deg;<sup>c</sup></p>`
...
async function get5DayForecast(city) {
  ...
  try {
    ...
    for (let i = 0; i < dateContainer.length; i++) {
      ...
      const formattedWeekday = currentDate.toLocaleDateString('en-US', { weekday: 'short' });
    ...
    for (let i = 0; i < weatherImg.length; i++) {
      switch (data.list[0].weather[0].main) {
        case "Clouds":
          ...
          break;
        case "Clear":
          weatherImg[i].src = "./assets/images/weather_icons/01d.png";
          break;
        case "Rain":
   ...
   // hourrly
function getHourlyData(city, data) {
  const currentTimestamp = Math.floor(Date.now() / 1000);

  // for (let i = 1; i <= 6; i++) {
  //   const targetTimestamp = currentTimestamp + i * 3600;
  //   const index = data.list.findIndex(entry => entry.dt >= targetTimestamp);
  ...
  //     const windformateTime = new Intl.DateTimeFormat('en-US', options).format(date);

  //     // Update existing hour elements
  //     document.getElementById(`hour${i}`).innerHTML = `<p> ${temperature}°C</p>`;
  //     document.getElementById(`timesec${i}`).innerHTML = ` ${formattedTime}`;
  //     document.getElementById(`windtime${i}`).innerHTML = `${ windformateTime}`;
  //     document.getElementById(`wind${i}`).innerHTML = `<p> ${windSpeed.toFixed(1)}km/h</p>`;
  //   }
  // }
...
// Uncomment the following line to enable hourly updates
getHourlyData(city, data);
...
function getSunrise(sun) {
  ...
  var seconds = sunriseDate.getSeconds();
      ...
      sunrise.innerHTML = hours+ ':' + (minutes<10 ? "0" : '') + minutes + '' + ampm;
}
...
function getSunset(set) {
  let sunset = document.getElementById("sunset");
      ...
      var seconds = sunsetDate.getSeconds();
          ...
          sunset.innerHTML = hours+ ':' + (minutes<10 ? "0" : '') + minutes + '' + ampm;
